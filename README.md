...
* **Indicadores de Desempenho (KPIs):** Monitoramento instantâneo do volume total de ocorrências, bairros afetados e status de resolução (Pendente, Em Andamento, Resolvido).
...
📦 projeto-python
 ...
 ┣ 📜 dados_defesa_social.csv  # Arquivo gerado automaticamente pelo gerador_dados.py
...
Antes de iniciar a interface visual, é necessário gerar os dados de teste. Execute o comando:

Bash
python gerador_dados.py
Nota: Aguarde a confirmação Arquivo 'dados_defesa_social.csv' gerado com sucesso! aparecer no terminal.

5. Iniciar o Dashboard Web
Para rodar a aplicação web, execute o comando abaixo. Utiliza-se o prefixo python -m para garantir que o sistema operacional localize o executável do Streamlit corretamente:
